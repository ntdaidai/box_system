# RepViT 默认分类模型 API 服务

基于 RepViT M1.1 默认预训练模型的图像分类服务。
...
```bash
cd /home/jetson/box_system/models/repvit-cls_default
...
```bash
curl -X POST http://localhost:8009/classify/image \
...
```bash
curl -X POST http://localhost:8009/classify/video \
  -H "Content-Type: application/json" \
