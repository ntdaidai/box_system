...
class ModelConfig:
    ...
    weights_path: str = "/app/models/yolo26x_smallobj_2_best.pt"
    device: str = "0"
    img_size: int = 960
    conf: float = 0.25
    iou: float = 0.45
    ...
    def __post_init__(self):
        if self.class_names is None:
            raw = os.getenv("CLASS_NAMES", "boat,swimmer,person,crowd").strip()
...
def load_config() -> tuple[MinIOConfig, ModelConfig, ServerConfig]:
    ...
    model_config = ModelConfig(
        weights_path=os.getenv("MODEL_WEIGHTS", "/app/models/yolo26x_smallobj_2_best.pt"),
