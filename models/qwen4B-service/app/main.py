...
def build_workflow_prompt(
    ...
    prompt = (
        ...
        "如果多条规则共同支撑同一个风险判断或处置建议，应全部引用；如果只有一条直接支撑，只引用该条。"
        "凡风险研判、影响评估、处置建议或持续监测使用了知识库条款，正文对应句子必须自然标注 [K数字] 引用；"
        ...
        "sentence_citations 只记录实际支撑正文结论的条款，不要把全部检索候选条款都当成已使用条款。"
        ...
        "正文对知识库条款可以使用准确的概括或改写，不要求逐字复制知识库原文；但必须明确写出‘根据知识库/条款/规定/依据’等来源关系。"
        "引用必须自然嵌入风险研判或处置理由，例如‘结合知识库依据[K数字]中关于下游禁入区域的规定，"
        "人员活动位于敏感区域时风险显著升高’；不要把引用孤立放在处置建议末尾。"
        "禁止输出‘将补充运行状态和知识库依据写入事件报告’、‘模型将依据知识库生成报告’等内部流程话术；"
        "处置建议只写现场人员能执行的动作，风险研判只写事实、适用条件和不确定性。"
