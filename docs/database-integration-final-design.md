...
当前实施采用增量兼容迁移：新表、新字段和新接口已启用，旧表与旧字段暂时保留供原接口运行。本报告中的“删除”均表示最终目标，尚未执行；删除前必须按第 7 节清单再次确认。
...
前端已只使用主键 `id`，展示 `camera_name`；后端自动测试大华、海康通道路径。旧 `camera_id` 与 `brand` 当前仅作兼容字段，待旧接口退出后删除。
...
删除：前端绘制业务 ID、矩形坐标、宽高、风险重复字段和 `trigger_seconds`。触发时间统一保存到条件库。
...
| `event_log` | 新触发已同步到 `safety_event_timeline_log`；旧表暂留兼容 |
| `safety_event_log` | 新视觉动作已同步到 `safety_event_timeline_log`；旧表暂留兼容 |
| 旧 `event_action` 执行字段 | 迁入时间线；`event_action` 只保留事件流程绑定 |
| `safety_event` | 拆为实例表和视觉详情表 |
| 旧 `safety_event_task` | 迁移为新的任务结构 |
| `sys_device` | 当前无实际摄像头引用，停止使用；传感器设备以后单独设计 |
| `sys_trigger_rule` | 不再使用，统一由 ECA 条件和事件处理 |
...
原视频安全闭环中的事件状态、风险变化、动作、任务和证据信息需要迁入新的实例、时间线、视觉详情、证据和任务表。

本次没有删除任何旧表、旧字段、旧代码或旧文件。已完成旧视频安全事件数据的兼容迁移；最终物理删除应在旧页面和小程序接口全部切换后另行执行。
