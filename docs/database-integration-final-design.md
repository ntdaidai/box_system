...
设备管理、视频运行态、区域接口和小程序均使用 `camera_device.id`；旧字符串业务编号及其兼容查询已删除。代码中的 `camera_id` 路径参数仅表示该主键，不对应额外数据库字段。后端自动测试大华、海康通道路径，`brand` 保存自动识别结果。
...
| camera_device_id | 可空，外键关联 `camera_device.id`，限定摄像头场景 |
| step_id | 外键，关联行为步骤 |
...
唯一约束：`(event_action_id, camera_device_id, step_id)`。后端校验广播设备已绑定对应摄像头，且 `step_id` 属于事件关联的流程。启用自动广播时设备和模板必填，启用无人机派飞时无人机和航线必填，不使用默认动作回退。
...
4. 迁移期先切换调用方，确认后删除旧 API；当前摄像头和安全闭环兼容接口已完成清理，不再保留双轨入口。
