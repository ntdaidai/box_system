...
| 旧 `event_action` 执行字段 | 广播、无人机和巡查报告仍读取，暂留兼容 |
| `safety_event` | 检测引擎、小程序和巡查报告仍读取，暂留兼容 |
| 旧 `safety_event_task` | 已补充实例外键和处置结果字段，当前任务服务仍使用 |
...
已删除旧安全闭环前端页面、旧设备/触发规则后端接口及其模型、Schema、未使用的前端 API 和历史 `.bak` 文件。广播测试设备已合并为唯一的“一号点广播”。其余活跃结构必须等检测引擎、小程序和巡查报告全部切换到统一实例表后再物理删除。
