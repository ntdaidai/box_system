  ...
  <div class="vision-page">
    <header class="command-header surface-card">
      ...
      <div class="header-status">
        <div class="status-pill" :class="modelStatus.loaded ? 'online' : 'offline'">
          <i></i>
          <span>{{ modelStatus.loaded ? 'AI 模型就绪' : '模型未加载' }}</span>
        </div>
    ...
    <section class="source-toolbar surface-card">
      ...
      <div class="toolbar-actions">
        ...
        >
          <el-icon><Aim /></el-icon>{{ detectionEnabled ? '停止 AI 检测' : '启动 AI 检测' }}
        </el-button>
        <el-button
          class="ghost-button"
          :disabled="!currentCamera?.connected || !modelStatus.loaded"
          @click="takeSnapshot"
        >
          <el-icon><Camera /></el-icon>截图检测
        </el-button>
      </div>
    </section>
    ...
    <section class="live-workspace">
      <article class="live-card surface-card">
        <div class="card-heading">
          ...
          <div class="feed-metrics">
            ...
            <span><small>显示模式</small><b>{{ detectionEnabled ? 'AI 标框' : '原始画面' }}</b></span>
        ...
        <div v-if="currentCamera?.connected" class="video-stage">
          ...
          />
          <svg
            v-if="detectionEnabled && imageWidth > 0 && imageHeight > 0"
            class="box-overlay"
      ...
      <aside class="telemetry-card surface-card">
        <div class="card-heading telemetry-heading">
          ...
          <span class="target-count">{{ detections.length }}<small>目标</small></span>
        </div>

        <div class="telemetry-grid">
          <div><small>检测状态</small><strong :class="detectionStatusClass">{{ detectionStatusText }}</strong></div>
          ...
          <div><small>检测帧</small><strong>#{{ latestDetection.frame_sequence || 0 }}</strong></div>
        </div>

        <div v-if="detectionEnabled && detections.length" class="target-list">
          <article
        ...
        <div v-else class="telemetry-empty">
          ...
          <p>{{ detectionEnabled ? '正在分析画面，当前未发现目标' : '启动 AI 检测后在此显示实时目标' }}</p>
        </div>
    ...
    <section class="media-lab surface-card">
      <div class="lab-heading">
        ...
        <el-tabs v-model="mediaTab" class="media-tabs">
          <el-tab-pane name="image">
            <template #label><span><el-icon><Picture /></el-icon>图片检测</span></template>
          </el-tab-pane>
          <el-tab-pane name="video">
            <template #label><span><el-icon><VideoPlay /></el-icon>视频检测</span></template>
          </el-tab-pane>
        </el-tabs>
      </div>
      ...
      <div v-show="mediaTab === 'image'" class="lab-content image-lab">
        ...
        >
          ...
          <h3>{{ imageUploading ? '正在执行图片推理' : '拖入一张图片开始检测' }}</h3>
        ...
        <div class="media-result image-result" :class="{ 'has-result': uploadResult }">
          <template v-if="uploadResult">
            <div class="image-preview-wrap">
              <img :src="`data:image/jpeg;base64,${uploadResult.result_image_base64}`" alt="图片检测结果" />
              <div class="result-caption">
                <span>识别到 <b>{{ imageDetections.length }}</b> 个目标</span>
            ...
            <aside class="image-target-panel">
              <div class="image-target-heading">
                ...
                <span>{{ imageDetections.length }}</span>
              </div>
              <div v-if="imageDetections.length" class="image-target-list">
                <article
              ...
              <div v-else class="image-target-empty">
                ...
                <strong>未发现目标</strong>
                <span>可以更换图片或降低检测阈值后重试</span>
              </div>
            </aside>
          </template>
          <div v-else class="result-placeholder"><el-icon><Picture /></el-icon><span>检测结果预览区</span></div>
        </div>
      </div>
      ...
      <div v-show="mediaTab === 'video'" class="lab-content video-lab">
        <div class="video-upload-column">
          ...
          <div v-if="videoResult" class="video-summary">
            ...
            <div><small>目标出现次数</small><strong>{{ videoResult.total_occurrences }}</strong></div>
          </div>
        </div>
        ...
        <div class="video-analysis-stage">
          <div v-if="videoPreviewUrl" class="uploaded-video-wrap">
            ...
            ></video>
            <svg
              v-if="videoSample && videoWidth > 0 && videoHeight > 0"
          ...
          <div v-else class="result-placeholder video-placeholder-result">
            <el-icon><VideoPlay /></el-icon><span>上传后可立即预览，检测完成后随播放同步标框</span>
          </div>

          <div v-if="videoResult" class="playback-targets">
            <div class="playback-heading">
              <span>当前播放位置检测</span>
              <b>{{ videoDetections.length }} 个目标</b>
            </div>
...
import {
  ...
  findVideoSample, isValidDetection, normalizeDetections,
...
const modelStatus = ref({ loaded: false, classes: {} })
...
const canToggleDetection = computed(() => Boolean(currentCamera.value?.connected && modelStatus.value.loaded))
...
const detectionStatusText = computed(() => {
  ...
  if (detectionConnectionState.value === 'connected') return '检测中'
...
const videoStateText = computed(() => ({
  ...
  completed: '检测完成', failed: '检测失败', cancelled: '已取消',
...
async function fetchModelStatus() {
  ...
  modelStatus.value = response.data || { loaded: false, classes: {} }
}
...
async function takeSnapshot() {
  const response = await snapshotDetect(currentCameraId.value, 0.5)
...
async function handleFileUpload(file) {
  ...
  try {
    ...
    const response = await detectImage(encoded, 0.5)
...
async function pollVideoJob() {
  ...
  try {
    ...
    if (response.data.state === 'completed') {
      ...
      syncVideoDetection()
      ElMessage.success('视频检测完成，可播放查看同步标框')
      return
    }
...
.source-control, .toolbar-actions { display: flex; align-items: center; gap: 10px; }
...
@media (max-width: 900px) {
  ...
  .header-status, .source-control, .toolbar-actions { flex-wrap: wrap; }
