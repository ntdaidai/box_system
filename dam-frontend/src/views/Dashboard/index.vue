          ...
          <div class="health-top-row">
            <div class="health-item health-item-medium">
              <div class="health-icon" style="background: rgba(100,200,255,0.15);">
                <el-icon :size="30" color="#64c8ff"><PlatformUptimeIcon /></el-icon>
              </div>
            ...
            <div class="health-item health-item-medium">
              <div class="health-icon" style="background: rgba(0,229,255,0.15);">
                <el-icon :size="30" color="#00e5ff"><HostUptimeIcon /></el-icon>
              </div>
              <div class="health-body">
          ...
          <div class="health-grid">
            ...
            <div class="health-item">
              <div class="health-icon" style="background: rgba(103,194,58,0.15);">
                <el-icon :size="26" color="#67c23a"><CpuLoadIcon /></el-icon>
              </div>
              <div class="health-body">
            ...
            <div class="health-item">
              <div class="health-icon" style="background: rgba(230,162,60,0.15);">
                <el-icon :size="26" color="#e6a23c"><MemoryModuleIcon /></el-icon>
              </div>
              <div class="health-body">
            ...
            <div class="health-item">
              <div class="health-icon" style="background: rgba(118,185,0,0.15);">
                <el-icon :size="26" color="#76b900"><GpuBoardIcon /></el-icon>
              </div>
              <div class="health-body">
            ...
            <div class="health-item">
              <div class="health-icon" style="background: rgba(245,108,108,0.15);">
                <el-icon :size="26" color="#f56c6c"><StorageStackIcon /></el-icon>
              </div>
              <div class="health-body">
            ...
            <div class="health-item">
              <div class="health-icon" :style="{ background: systemInfo.sensor_collector_running ? 'rgba(103,194,58,0.15)' : 'rgba(245,108,108,0.15)' }">
                <el-icon :size="26" :color="systemInfo.sensor_collector_running ? '#67c23a' : '#f56c6c'"><EdgeCollectorIcon /></el-icon>
              </div>
              <div class="health-body">
            ...
            <div class="health-item">
              <div class="health-icon" :style="{ background: aiModelHealthy ? 'rgba(103,194,58,0.15)' : 'rgba(245,108,108,0.15)' }">
                <el-icon :size="26" :color="aiModelHealthy ? '#67c23a' : '#f56c6c'"><AiInferenceIcon /></el-icon>
              </div>
              <div class="health-body">
...
const setDeviceOnlineState = (key, online, extra = {}) => {
  deviceStatus.value = {
    ...deviceStatus.value,
    [key]: {
      ...(deviceStatus.value[key] || {}),
      ...extra,
...
const fetchCameraSummaryStatus = async () => {
  try {
    const res = await getCameraList()
    const cameras = res.data?.cameras || []
    const onlineCount = cameras.filter(camera => camera.connected).length
    setDeviceOnlineState('camera', onlineCount > 0, {
      total: cameras.length,
      online_count: onlineCount,
      channel_status: onlineCount > 0 ? `${onlineCount}/${cameras.length || onlineCount}` : '--',
    })
  } catch {
    setDeviceOnlineState('camera', false, { channel_status: '--' })
  }
}

const ensureDijToken = async () => {
  ...
  if (nextToken) localStorage.setItem('dij_token', nextToken)
  return nextToken
}
...
const flattenDroneTopology = (nodes = []) => {
  ...
  nodes.forEach((node) => {
    ...
    if (Array.isArray(children)) result.push(...flattenDroneTopology(children))
    else if (children && typeof children === 'object') result.push(...flattenDroneTopology(Object.values(children)))
  })
  return result
}

const fetchDroneSummaryStatus = async () => {
  try {
    await ensureDijToken()
    const workspaceRes = await getCurrentWorkspace()
    const workspaceId = workspaceRes.data?.workspace_id || workspaceRes.data?.id || ''
    if (!workspaceId) throw new Error('缺少 DJI 工作空间')

    let devices = []
    try {
      const topologyRes = await getDroneDevices(workspaceId)
      devices = flattenDroneTopology(topologyRes.data || [])
    } catch {
      const boundRes = await getBoundDevices(workspaceId, { page: 1, page_size: 50 })
...
const initData = async () => {
  ...
  await Promise.all([
    ...
    fetchCameraSummaryStatus(),
    fetchDroneSummaryStatus(),
  ])
...
const formatSensorValue = (key, field) => {
  ...
  if (field === 'channel_status') return deviceStatus.value[key]?.channel_status || (getSensorOnline(key) ? '待检测' : '--')
  if (field === 'position_status') return deviceStatus.value[key]?.position_status || (getSensorOnline(key) ? '已接入' : '--')
