    ...
    public HttpResultResponse liveStart(LiveTypeDTO liveParam) {
        ...
        switch (liveParam.getUrlType()) {
            case AGORA:
                break;
            case RTMP:
                // 将 rtmp://ip:1936/live/xxx 转换为 webrtc://ip:8890/live/xxx
                String rtmpUrl = url.toString();
                String webrtcUrl = rtmpUrl.replace("rtmp", "webrtc").replace(":1936", ":8890");
            ...
            case GB28181:
                ...
                break;
            case RTSP:
                Object output = response.getData().getOutput();
                live.setUrl(output != null ? output.toString() : url.toString());
                break;
            case WHIP:
